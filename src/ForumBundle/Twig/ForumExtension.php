...
class ForumExtension extends \Twig_Extension
{
    ...
    {
        $posts = $this->em->getRepository('ForumBundle:Post')->findBy(
            array('poster' => $poster),
            array('date' => 'DESC'),
            $limit,
            null);
    ...
    {
        $topics = $this->em->getRepository('ForumBundle:Topic')->findBy(
            array('user' => $poster),
            array('date' => 'DESC'),
            $limit,
            null);
    ...
    public function dfLastPostsEdited($limit = null)
    {
        ...
        $posts = $this->em->getRepository('ForumBundle:Post')->findLastEdited($limit);
    ...
    {
        $topic = $this->em->getRepository('ForumBundle:Topic')->findOneBy(
    ...
    {   
        ...
        if ($paginatorEnabled === true) {
            ...
            if ($pagesCount > 1) {
                return $this->router->generate('forum_post', array(
                    'slug'     => $topic->getSlug(),
                    $query => $pagesCount
                ));
            }
        ...
        return $this->router->generate('forum_post', array('slug' => $topic->getSlug()));
    }

    public function getName()
    {
        return 'forumbundle_extension';
    }
